...
import static com.lld.parkinglot.model.parking.ParkingFloor.getSpotTypeForVehicle;
    ...
    public boolean isFull() {
        for (ParkingFloor parkingFloor : parkingFloors) {
            if (!parkingFloor.isFloorFull()) {
                return false; // At least one floor is not full
            }
        }
        return true; // All floors are full
    }
